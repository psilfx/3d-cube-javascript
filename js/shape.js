...
class Shape3D {
	...
	CreateTriangles() {
		...
		for( let p = 0; p < indices.length; p += 3 ) {
			...
			let triangle = new Triangle( [ point1  , point2  , point3 ] , [ normal1 , normal2 , normal3 ] );
				triangle.CalcNormal();
	...
	ApplyTransforms() {
		...
		mat4.Scale( scaleMatrix , new Vector3F( this.scale.x , this.scale.y , this.scale.z ).Normalize() );
		...
		for( let t = 0; t < triangles.length; t++ ) {
			...
			let direction    = triangle.fragsTransform[ 0 ].Subtract( camera.eye ).Normalize();
			if( direction.Dot( triangle.normalTransform ) > 0 ) continue; //Откидываем невидимый треугольник
